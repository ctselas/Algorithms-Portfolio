...
class Classification_Performance(object):
    ...
    def __init__(self,target, predicted, name, flag = 0):
        ...
        self.document.add_heading('Results for Decision Tree with max depht 7 and 10 fold CV') 
        
        self.paragraph = self.document.add_paragraph()#create paragraph to save this print  
        self.paragraph.text = '\n=====================================\n'
        self.paragraph.text = self.paragraph.text + 'Performance of Dataset: ' + self.name +'\n'
        print('\n=====================================')
    ...
    def area_under_the_ROC(self):
        ...
        self.paragraph.text = self.paragraph.text + 'False Positive Rate (FPR = FP/N = ‘Normals predicted as anomalies’/ ‘Normals’):' + '\n'
        self.paragraph.text = self.paragraph.text + '-> FPR = ' + str(round(fpr[1],2)) + '\n'
        self.paragraph.text = self.paragraph.text + 'True Positive Rate (TPR= TP/P = ‘Anomalies predicted correctly’ / ‘Anomalies’):'  + '\n'
        self.paragraph.text = self.paragraph.text + '-> TPR = '+ str(round(tpr[1],2)) + '\n'
        self.paragraph.text = self.paragraph.text + 'Mean Area Under the ROC: ' + str(round(roc_auc,2)) + '\n'
#        
        fig, ax = plt.subplots()        
        ...
        ax.text(fpr[1] , tpr[1] + 0.05 , textstr, transform=ax.transAxes, fontsize=14,verticalalignment='top', color ='b')   
